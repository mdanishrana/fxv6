...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        // --- NEW: Initial Billing Generation ---
        if (newCattle.monthly_charges > 0 && newCattle.entry_date) {
            try {
                await db.query(`
                    INSERT INTO payments (
                        tenant_id, cattle_id, amount, due_date, status, notes
                    ) VALUES ($1, $2, $3, $4, 'PENDING', 'Initial 30-day Cycle')
                `, [req.tenantId, newCattle.id, newCattle.monthly_charges, newCattle.entry_date]);
                console.log(`Auto-created initial payment cycle for animal ${newCattle.id}`);
            } catch (billErr) {
                console.error('Failed to auto-create initial bill:', billErr);
            }
        }
