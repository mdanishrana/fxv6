...
import { Pill, Syringe, ShieldAlert } from 'lucide-react';
import { useTheme } from '../services/ThemeContext';
import { MedicalManager } from './MedicalManager';
import { VaccinationReport } from './reports/VaccinationReport';
import { Cattle } from '../types';

type HealthTab = 'INVENTORY' | 'VACCINATIONS';
...
export const HealthManager: React.FC<HealthManagerProps> = ({ tenantId, cattle }) => {
    const { isRTL } = useTheme();
    const [activeTab, setActiveTab] = useState<HealthTab>('INVENTORY');

    const PAKISTAN_PROTOCOLS = [
        {
    ...
    return (
        <div className="space-y-6 animate-fade-in relative z-10 w-full">
            {/* Header Tabs */}
            <div className="flex gap-2 p-1 bg-white dark:bg-slate-800/50 rounded-xl overflow-x-auto w-fit mb-2 border border-slate-200 dark:border-slate-700">
                <button
                    onClick={() => setActiveTab('INVENTORY')}
                    className={`px-5 py-2.5 rounded-lg font-bold text-sm flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'INVENTORY'
                        ? 'bg-rose-100 dark:bg-rose-900/40 text-rose-700 dark:text-rose-300 shadow-sm'
                        : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'
                        }`}
                >
                    <Pill size={18} /> Medical Inventory
                </button>
                <button
                    onClick={() => setActiveTab('VACCINATIONS')}
                    className={`px-5 py-2.5 rounded-lg font-bold text-sm flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'VACCINATIONS'
                        ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 shadow-sm'
                        : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'
                        }`}
                >
                    <Syringe size={18} /> Vaccinations & Protocols
                </button>
            </div>

            {/* Active Render Area */}
            {activeTab === 'INVENTORY' && (
                <div className="-mx-4 sm:mx-0 lg:-mx-8">
                    {/* The existing MedicalManager component handles its own layout and fetching */}
                    <MedicalManager tenantId={tenantId} />
                </div>
            )}
            ...
            {activeTab === 'VACCINATIONS' && (
                <div className="space-y-8">
                    ...
                    <VaccinationReport cattle={cattle} tenant={{name: 'Farm'}} />
                </div>
            )}
        </div>
    );
